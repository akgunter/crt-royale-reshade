#ifndef _PHOSPHOR_MASK_RESIZING_H
#define _PHOSPHOR_MASK_RESIZING_H
...
static const float max_sinc_resize_samples_m4 = root_ceil(
    ...
    const bool vertical)
{
    ...
    const float2 first_texel = prev_texel - float2(samples/2.0 - 1.0, samples/2.0 - 1.0);
    ...
    #define CALCULATE_R_COORD_FOR_4_SAMPLES                                    \
        ...
        #define CALCULATE_SINC_RESAMPLE_WEIGHTS                                \
            ...
            const float4 weights = min(sin(pi_dist) * sin(pi_dist_over_lobes) /\
                (pi_dist*pi_dist_over_lobes), float4(1.0, 1.0, 1.0, 1.0));
    #else
        #define CALCULATE_SINC_RESAMPLE_WEIGHTS                                \
            const float4 weights = min(sin(pi_dist)/pi_dist, float4(1.0, 1.0, 1.0, 1.0));
    #endif
    ...
    const float magnification_scale, const float tile_size_uv_r)
{
    ...
    int i_base = 0;
    float4 weight_sum = float4(0.0, 0.0, 0.0, 0.0);
    ...
    const float scalar_weight_sum_raw = weight_sum_reduce.x + 
        weight_sum_reduce.y;
    const float3 scalar_weight_sum = float3(scalar_weight_sum_raw, scalar_weight_sum_raw, scalar_weight_sum_raw);
    ...
    out float2 mask_tiles_per_screen)
{
    ...
    else
    {
        ...
        static const float2 mask_tile_uv_size = float2(1.0, 1.0);
        static const float2 mask_tile_start_uv = float2(0.0, 0.0);
...
#endif  //  _PHOSPHOR_MASK_RESIZING_H