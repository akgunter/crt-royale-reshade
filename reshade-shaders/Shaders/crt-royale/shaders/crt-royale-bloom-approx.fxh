...
float3 tex2Dresize_gaussian4x4(sampler2D tex, float2 tex_uv, float2 dxdy, float2 tex_size, float2 texture_size_inv, float2 tex_uv_to_pixel_scale, float sigma, float input_gamma)
{
    ...
    //  Load each sample:
    float3 sample0 = tex2D_linearize(tex, sample0_uv, input_gamma).rgb;
    float3 sample1 = tex2D_linearize(tex, sample1_uv, input_gamma).rgb;
    float3 sample2 = tex2D_linearize(tex, dx, input_gamma).rgb;
    float3 sample3 = tex2D_linearize(tex, sample3_uv, input_gamma).rgb;
    float3 sample4 = tex2D_linearize(tex, sample4_uv, input_gamma).rgb;
    float3 sample5 = tex2D_linearize(tex, sample5_uv, input_gamma).rgb;
    float3 sample6 = tex2D_linearize(tex, sample6_uv, input_gamma).rgb;
    float3 sample7 = tex2D_linearize(tex, sample7_uv, input_gamma).rgb;
    float3 sample8 = tex2D_linearize(tex, sample8_uv, input_gamma).rgb;
    float3 sample9 = tex2D_linearize(tex, sample9_uv, input_gamma).rgb;
    float3 sample10 = tex2D_linearize(tex, sample10_uv, input_gamma).rgb;
    float3 sample11 = tex2D_linearize(tex, sample11_uv, input_gamma).rgb;
    float3 sample12 = tex2D_linearize(tex, sample12_uv, input_gamma).rgb;
    float3 sample13 = tex2D_linearize(tex, sample13_uv, input_gamma).rgb;
    float3 sample14 = tex2D_linearize(tex, sample14_uv, input_gamma).rgb;
    float3 sample15 = tex2D_linearize(tex, sample15_uv, input_gamma).rgb;
...
void pixelShader2(
    ...
    const float2 vertical_scanlines_size = tex2Dsize(samplerVerticalScanlines);
...
//    const float2 blur_dxdy = blur_dxdy;
    // const float2 texture_size_ = tex2Dsize(samplerOrigLinearized);
...
//    const float2 tex_uv_to_pixel_scale = tex_uv_to_pixel_scale;
    ...
    if(bloom_approx_filter > 1.5)
    {
        ...
        if(beam_misconvergence)
        {
            out_color_r = tex2Dresize_gaussian4x4(samplerOrigLinearized, tex_uv_r,
                blur_dxdy, orig_linearized_size, texture_size_inv,
                tex_uv_to_pixel_scale, bloom_approx_sigma, get_intermediate_gamma());
            out_color_g = tex2Dresize_gaussian4x4(samplerOrigLinearized, tex_uv_g,
                blur_dxdy, orig_linearized_size, texture_size_inv,
                tex_uv_to_pixel_scale, bloom_approx_sigma, get_intermediate_gamma());
            out_color_b = tex2Dresize_gaussian4x4(samplerOrigLinearized, tex_uv_b,
                blur_dxdy, orig_linearized_size, texture_size_inv,
                tex_uv_to_pixel_scale, bloom_approx_sigma, get_intermediate_gamma());
        }
        else
        {
            out_color = tex2Dresize_gaussian4x4(samplerOrigLinearized, tex_uv,
                blur_dxdy, orig_linearized_size, texture_size_inv,
                tex_uv_to_pixel_scale, bloom_approx_sigma, get_intermediate_gamma());
        }
    }
    else if(bloom_approx_filter > 0.5)
    {
        ...
        if(beam_misconvergence)
        {
            out_color_r = tex2Dblur3x3resize(samplerOrigLinearized, tex_uv_r,
                blur_dxdy, bloom_approx_sigma, get_intermediate_gamma());
            out_color_g = tex2Dblur3x3resize(samplerOrigLinearized, tex_uv_g,
                blur_dxdy, bloom_approx_sigma, get_intermediate_gamma());
            out_color_b = tex2Dblur3x3resize(samplerOrigLinearized, tex_uv_b,
                blur_dxdy, bloom_approx_sigma, get_intermediate_gamma());
        }
        else
        {
            out_color = tex2Dblur3x3resize(samplerOrigLinearized, tex_uv, blur_dxdy, get_intermediate_gamma());
        }
    }
    else
    {
        ...
        if(beam_misconvergence)
        {
            out_color_r = tex2D_linearize(samplerOrigLinearized, tex_uv_r, get_intermediate_gamma()).rgb;
            out_color_g = tex2D_linearize(samplerOrigLinearized, tex_uv_g, get_intermediate_gamma()).rgb;
            out_color_b = tex2D_linearize(samplerOrigLinearized, tex_uv_b, get_intermediate_gamma()).rgb;
        }
        else
        {
            out_color = tex2D_linearize(samplerOrigLinearized, tex_uv, get_intermediate_gamma()).rgb;
        }
    }
